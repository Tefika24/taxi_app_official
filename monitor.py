...
class Monitor:
    ...
    def __str__(self) -> str:
        ...
        """
        d = len(self._activities[DRIVER])
        r = len(self._activities[RIDER])
        return f"Monitor ({d} drivers, {r} riders)"
    ...
    def _average_total_distance(self) -> float:
        ...
        dist = 0.0
        count = 0
        for acts in self._activities[DRIVER].values():
            for i in range(len(acts) - 1):
                a1 = acts[i].location
                a2 = acts[i + 1].location
                distance = manhattan_distance(a1, a2)
                dist += distance
            count += 1
        # print(f"{dist} \ {count}")
        return dist / count
    ...
    def _average_ride_distance(self) -> float:
        ...
        for acts in self._activities[DRIVER].values():
            for i in range(len(acts) - 1):
                if acts[i].description == PICKUP:
                    a1 = acts[i].location
                    a2 = acts[i + 1].location
                    dist += manhattan_distance(a1, a2)
                    count += 1
                    print(a1, a2)
        # print(f"{dist} \ {count}")
        return dist / count
